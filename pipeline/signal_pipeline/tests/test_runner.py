...
from signal_pipeline.src.kalman_source import KalmanSignalSource
...
class TestGenerateAndPublish:
    ...
    def test_publishes_when_candles_available(self) -> None:
        ...
        with patch("signal_pipeline.src.runner._fetch_candles", return_value=_rising_candles(60)):
            _generate_and_publish("BTCUSDT", src, prod)
    ...
    def test_skips_when_insufficient_for_features(self) -> None:
        src = MagicMock(spec=BaseSignalSource)
        prod = MagicMock()
        with patch("signal_pipeline.src.runner._fetch_candles", return_value=_rising_candles(10)):
            _generate_and_publish("BTCUSDT", src, prod)
        src.predict.assert_not_called()
...
class TestBuildSources:
    ...
    def test_kalman_only(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("signal_pipeline.src.runner.SIGNAL_SOURCE", "kalman")
        sources = _build_sources()
        assert set(sources.keys()) == {"kalman"}
        assert isinstance(sources["kalman"], KalmanSignalSource)

    def test_comma_separated_list(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("signal_pipeline.src.runner.SIGNAL_SOURCE", "ml,kalman")
        sources = _build_sources()
        assert set(sources.keys()) == {"ml", "kalman"}

    def test_comma_separated_list_with_all_three(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("signal_pipeline.src.runner.SIGNAL_SOURCE", "ml,llm,kalman")
        sources = _build_sources()
        assert set(sources.keys()) == {"ml", "llm", "kalman"}
    ...
    def test_partially_invalid_list_raises_value_error(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr("signal_pipeline.src.runner.SIGNAL_SOURCE", "ml,bogus")
        with pytest.raises(ValueError, match="SIGNAL_SOURCE"):
            _build_sources()

    def test_empty_string_raises_value_error(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("signal_pipeline.src.runner.SIGNAL_SOURCE", "")
        with pytest.raises(ValueError, match="SIGNAL_SOURCE"):
            _build_sources()
...
class TestFetchCandles:
    def test_returns_empty_on_db_error(self) -> None:
        with patch(
            "signal_pipeline.src.runner.get_db_connection", side_effect=DatabaseError("no db")
        ):
